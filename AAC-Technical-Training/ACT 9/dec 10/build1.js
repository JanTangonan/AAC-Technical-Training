function panelRead(requestData, onSuccess, onError) {
    $.ajax({
        ...
        success: function (response) {
            var data = JSON.parse(response.d); // Parse the response data
            onSuccess(data); // Pass the data to the success callback
        },
...
$(document).ready(function () {
    ...
    function edit() {
        ...
        if (controlPanel.dataKey) {
            console.log(controlPanel.dataKey);
            console.log(grid.selectedDataKey);

            //controlPanel.dataKey = grid.dataKey;
