...
function handleSubmit() {
    ...
    $.ajax({
        type: "POST",
        url: "Exercise_Arvinn_AjaxWS.asmx/GetReversedCsv", // Replace with your WebMethod URL
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ csvInput: csvContent }),
        dataType: "json",
        success: function (response) {
            alert('Data successfully sent to the server!');
            console.log(response.d); // Output the response from the WebMethod
...
function displayReversedTable(reversedData) {
    // Create a new table for displaying the reversed data
    ...
    // Populate table rows with reversed data
...
$(document).ready(() => {
    ...
    function generateTable() {
        ...
        for (var i = 0; i < numRows; i++) {
            ...
            row.innerHTML = `
                <td><input type="text" placeholder="First Name" onkeypress="return blockComma(event)"/></td>
                <td><input type="text" placeholder="Middle Name" onkeypress="return blockComma(event)"/></td>
                <td><input type="text" placeholder="Last Name" onkeypress="return blockComma(event)"/></td>
                <td>
                    <select>
