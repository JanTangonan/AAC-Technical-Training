(function ($) {
    ...
    $.fn.dbPanel = function (options) {
        ...
        function renderControls() {
            ...
            settings.controls.forEach(control => {
                ...
                if (control.type === 'text') {
                    ...
                    if (control.data) {
                        bindDropdown(control.data, $input, control.valueField, control.textField);
                    }
        ...
        function bindDropdown(data, $dropdown, valueField, textField) {
...
$(document).ready(function () {
    ...
    let controlPanel = $('.dbControlPanel').data('dbPanel');
    let chargesData = [];
    let departmentsData = [];

    // Fetch all required data before initializing the controls
    function preloadData() {
        const fetchChargesPromise = $.ajax({
            type: "POST",
            url: "Exercise_Arvinn_AjaxWS.asmx/GetCharges",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                chargesData = JSON.parse(response.d);
        ...
        const fetchDepartmentsPromise = $.ajax({
            type: "POST",
            url: "Exercise_Arvinn_AjaxWS.asmx/GetDepartments",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (response) {
                departmentsData = JSON.parse(response.d);
        ...
        // Return a promise that resolves once both AJAX calls complete
        return Promise.all([fetchChargesPromise, fetchDepartmentsPromise]);
    }


    preloadData().then(() => {
        $('.dbControlPanel').dbPanel({
            controls: [
