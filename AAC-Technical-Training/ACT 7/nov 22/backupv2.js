(function ($) {
    ...
    $.fn.dbPanel = function (options) {
        ...
        function renderControls() {
                ...
                if (control.type === 'text') {
                    ...
                    if (control.dataSourceUrl) {
                        ...
                        fetchDropdownData(control.dataSourceUrl, $input, control.valueField, control.textField, () => {
                            pendingDropdowns--; // Decrement pending count
            ...
            // If no dropdowns to populate, trigger callback immediately
        ...
        function checkAllDropdownsPopulated() {
            /// <summary>Checks if all dropdowns are populated and triggers the callback</summary>
            /// <param>onRendered: Function to execute once all dropdowns are ready.</param>
            if (pendingDropdowns === 0) {
                settings.onRendered?.(); // Trigger callback if all dropdowns are ready
            }
        }
...
$(document).ready(function () {
    ...
    const controlPanel = $('.dbControlPanel').dbPanel({
        ...
        onRendered: function () {
            ...
            /// <param></param>
            /// <return type = "void">Void.</return>
            $(".dbgCasesTable").off('click', 'tr').on('click', 'tr', function () {
                const selectedRecord = $(this).data('record');
                if (selectedRecord) {
                    controlPanel.loadData(selectedRecord);
                }
            });

            /// trigger click for the first row
    ...
    $(".dbgCasesTable").on('click', 'tr', function () {
        const selectedRecord = grid.getData();
        if (selectedRecord) {
            controlPanel.loadData(selectedRecord);
        }
    });
