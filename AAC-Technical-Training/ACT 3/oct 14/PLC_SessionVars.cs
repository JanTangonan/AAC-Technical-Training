...
namespace PLCCONTROLS
{
    ...
    [Serializable]
    public enum CaseSearchType { None, ByCase, ByName, ByReference, ByAttribute, ByTestType, CODISSearch, CustomSearch, ByRecentCases, ByMyAssignments };
...
(i.CASE_KEY = ?)"
        ...
        public string GetConnectionString(string dbnameOverride, string datasourceOverride)
        {
            ...
            else
            {
                ...
                if (PLCDatabaseServer == "MSSQL")
                {
                    ...
                    if (PLCDBProvider == "MSOLEDBSQL")
                    {
                        //DataTypeCompatibility=80; needed to support DATETIME COLUMN TYPES

                        tempstr = "Provider=MSOLEDBSQL;Server={0}; DataTypeCompatibility=80; Database={1}; User ID={2}; Password={3};";


                        tempstr = String.Format(tempstr, theDataSource, theDatabaseName, PLCDBUserID, PLCDBPW);
                    }
                    else
                    {
        ...
        private void WriteDebug(string Msg, bool TimeStamp, bool bAlwaysLog)
        {
            try
            {
                ...
                {
                    string FileName = PLCPath.LabSave + "webdebug-" + (isPrelogMode ? prelogDebugName : "") + DateTime.Today.Date.Month + "-" + DateTime.Today.Date.Day + "-" + DateTime.Today.Date.Year + "(" + (isPrelogMode ? PLCGlobalPrelogUser : PLCGlobalAnalyst) + ").TXT";
...
WHERE I.EVIDENCE_CONTROL_NUMBER = " + ecn);
        ...
        private string GetLabCtrlDb(string labctrlkey, string labcode)
        {
            ...
            if (labctrlTable != null)
            {
                ...
                if ( qry.HasData() && (qry.FieldExist(labctrlkey)) )
                    return qry.FieldByName(labctrlkey);
                else
                    return "";
            }
            else
            ...
            */
            ...
            if (isPrelog)
            {
                ...
                qryLog.SQL = String.Format("INSERT INTO TV_AUDITWEB " +
                   ...
                   "OS_ADDRESS, ADDITIONAL_INFORMATION{0}{1}{2}{3}{4}{5}) " +
        ...
        public string GenerateCodeHeadSQL(string tableName, string selectedValue, string filter, string descFormat, string descSeparator, string parentFlexBoxValue, bool showActiveOnly, string sortOrder, Dictionary<string, object> parentControlValues = null)
        {
            ...
            if (showActiveOnly && (tableName == "TV_ANALYST" || tableName == "CV_ANALYST"))
                filter += " AND (ACCOUNT_DISABLED <> 'T' OR ACCOUNT_DISABLED IS NULL)";
            else if (tableName == "TV_CONTAINER")
                filter += " AND (DELETED <> 'T' OR DELETED IS NULL)";
            ...
            if (!string.IsNullOrEmpty(selectedValue))
            {
                ...
                filter += string.Format(" AND {0} IN ('{1}')", codeField, selectedValue);
            }

            if (showActiveOnly && !string.IsNullOrEmpty(activeField))
                filter += " AND (ACTIVE <> 'F' OR ACTIVE IS NULL)";

            if (!string.IsNullOrEmpty(filter))
                sql += " WHERE" + filter.Remove(0, 4);
        ...
        public string GetStoredProcDecryptValue(string codeValue)
        {
            ...
            try
            {
                ...
                if (PLCSession.PLCDatabaseServer == "ORACLE")
                {
                    ...
                    qry.AddProcedureParameter("o_value", "", 50, OleDbType.VarChar, ParameterDirection.Output);
        ...
        public string GetStoredProcEncrypt(string codeValue)
        {
            ...
            try
            {
                ...
                if (PLCSession.PLCDatabaseServer == "ORACLE")
                {
                    qry.AddProcedureParameter("p_codeValue", codeValue, 50, OleDbType.VarChar, ParameterDirection.Input);
                    qry.AddProcedureParameter("p_passPhrase", passPhrase, 100, OleDbType.VarChar, ParameterDirection.Input);
        ...
        {
            string setting = PLCSession.GetUserPref("SYS_LANGUAGE");
