...
BEGIN
        ...
        BEGIN
            ...
            IF create_initial_receipt = 'T' THEN
                ...
                BEGIN
                    ...
                    output_header := TO_CHAR(quantity) || ' ' || quantity_units || ' of the chemical ' ||
                                       assettyp_description_t || ' stored in ' || lot_number_t;
                END;
            ELSE
                DECLARE
    ...
    ELSIF chem_category = 'F' THEN
        ...
        BEGIN
            ...
            IF LENGTH(output_header) > 99 THEN
                output_header := SUBSTR(output_header, 1, 99) || '...';
            END IF;
        END;
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        output_header := 'No data found for control number: ' || chem_control_number;
    WHEN OTHERS THEN
        output_header := 'Error: ' || SQLERRM;
