USE [VENTURA_DEV]
GO
/****** Object:  StoredProcedure [dbo].[CHEMINV_GET_HEADER]    Script Date: 2/21/2025 2:28:45 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    BEGIN
        SELECT @output_header = 
			STUFF(
            (SELECT ' >> ' + ISNULL(B.DESCRIPTION, 'Unknown') + ' - ' + ISNULL(C.DESCRIPTION, 'Unknown')
             FROM TV_CHEMSTAT A
             ...
             ORDER BY A.STATUS_DATE, A.STATUS_TIME
             FOR XML PATH(''), TYPE).value('.', 'NVARCHAR(MAX)'), 
        1, 4, '');

		IF LEN(@output_header) > 75
		BEGIN
			SET @output_header = LEFT(@output_header, 97) + '...';
		END
    END
    ELSE
    BEGIN
