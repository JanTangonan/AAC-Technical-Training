...
BEGIN
    ...
    ELSIF v_chem_category = 'D' THEN
        BEGIN
            SELECT LISTAGG(NVL(B.DESCRIPTION, 'Unknown') || ' - ' || NVL(C.DESCRIPTION, 'Unknown'), ' >> ') 
               WITHIN GROUP (ORDER BY A.STATUS_DATE, A.STATUS_TIME)
            INTO p_output_header
            FROM TV_CHEMSTAT A
            ...
            IF LENGTH(p_output_header) > 75 THEN
              p_output_header := SUBSTR(p_output_header, 1, 97) || '...';
            END IF;
        END;
    ELSE
        p_output_header := '';
