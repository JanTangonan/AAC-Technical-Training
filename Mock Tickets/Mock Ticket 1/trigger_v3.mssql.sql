...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT @max_suffix = MAX(SUBSTRING(LAB_ITEM_NUMBER, LEN(@lab_item_number_prefix) + 1, 1))
            FROM TV_LABITEM
            WHERE LAB_ITEM_NUMBER LIKE @lab_item_number_prefix + '[a-z]';

            -- Generate the new suffix
            IF @max_suffix IS NULL
                SET @suffix = 'a';
            ELSE
                SET @suffix = CHAR(ASCII(@max_suffix) + 1);

            -- Generate the new LAB_ITEM_NUMBER
            SET @new_lab_item_number = @lab_item_number_prefix + @suffix;
